"""Local Cool Bool implementation used by Carve Along Path.

This is intentionally kept inside the Carve package so its subtraction path
uses the same island selection, normal handling, invert behavior, and cleanup
as the standalone Cool Bool tool.
"""
...
class MESH_OT_CoolBool(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        try:
            solver_mode = getattr(self, "_solver_mode", None)
...
class _CoolBoolExecution:
    """Property container for running the copied Cool Bool core in Carve.

    Carve owns the visible operator and its redo panel, so invoking a second
    registered Blender operator would replace Carve's redo state.  This small
    proxy lets Carve call the copied Cool Bool execute path directly while
    retaining the same island separation, Boolean, normal cleanup, and
    ...
    def __init__(self, invert_cutter, solver_mode):
        ...
        self.invert_cutter = invert_cutter
        self.merge_subtract_results = False
        self.limited_dissolve = True
        self.dissolve_per_iteration = True
